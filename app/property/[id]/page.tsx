...
export default async function PropertyDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const property = gridFake.find((p) => p.id === id);

  if (!property) {
    return <div className="p-8">Propriedade não encontrada</div>;
  }

  const gallery = property.images ?? [property.imageUrl, property.imageUrl, property.imageUrl, property.imageUrl, property.imageUrl]
      ...
      <div className="grid grid-cols-3 gap-8 mt-8">
        ...
        <div className="col-span-2">
          ...
          <div className="flex items-center gap-3 text-neutral-600">
            ...
            <div>
              ...
              {property.isSuperhost && <p className="text-sm text-neutral-500">Superhost</p>}
            </div>
          </div>
        ...
        <div>
          <div className="border rounded-xl shadow-lg p-6 sticky top-8">
            ...
            <p className="text-center text-sm text-neutral-500 mt-2">Você ainda não será cobrado</p>
          </div>
        </div>
      </div>
    </div>
